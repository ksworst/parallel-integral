...
int main() {
  ...
  //execution time
   ...
   integral2<<< number_of_blocks, threads_per_block, 0, stream1 >>>(x_1, x_2, d, result);
   cudaDeviceSynchronize();
   cudaStreamDestroy(stream1); //Deallocates a stream
   ...
   *result=0;
   ...
   integral3<<< number_of_blocks, threads_per_block, 0, stream2 >>>(x_1, x_2, d, result);
   cudaDeviceSynchronize();
   cudaStreamDestroy(stream2); //Deallocates a stream
